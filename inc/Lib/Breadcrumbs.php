...
class Breadcrumbs {
	...
	/**
	 * Separator between breadscrumbs list.
	 *
	 * @var	string
     * @access private
     * @since  1.0
     ...
     * @static
     * @access public
     * @return AuroraThemes\Base\Breadcrumbs
     * @since  1.0
     */
	...
	public function get_breadcrumbs( array $args ) {
		...
		$defaults = [
			...
			'home_text'             		=> sprintf( '<i class="fa fa-home"></i> <span>%s</span>', esc_attr__( 'Home', 'aurora-wp' ) ),
			'cat_archive_prefix'     		=> esc_attr__( 'Category:&nbsp;', 'aurora-wp' ),
			'tag_archive_prefix'     		=> esc_attr__( 'Tag:&nbsp;', 'aurora-wp' ),
			'search_prefix'          		=> esc_attr__( 'Search query for:&nbsp;', 'aurora-wp' ),
			'error_prefix'           		=> esc_attr__( '404 - Page not found', 'aurora-wp' ),
		];

		$defaults     						= apply_filters( 'aurora_breadcrumbs_defaults', $defaults );
		...
		// Check if the Wordpress SEO options is activated.
	...
	private function start_breadcrumb_rendering() {
		...
		// Woocommerce support.
		if ( class_exists( 'WooCommerce' ) && ( ( aurora_inside_woocommerce() && is_archive() && ! is_shop() ) || is_cart() || is_checkout() || is_account_page() ) ) {
		...
		if ( is_singular() ) {
			...
			} else if ( ( is_tax() || is_tag() || is_category() || is_date() || is_author() ) && $this->display_post_type_archive && ! aurora_inside_woocommerce() ) {
				$this->render_markup .= $this->get_post_type_archive();
			}
			...
			if ( is_post_type_archive() ) {
				...
				// 404 Page.
				if ( aurora_inside_tribe_event() || ( is_post_type_archive( 'tribe_events' ) || ( aurora_inside_tribe_event() && is_archive() ) ) ) {
	...
	private function breadcrumbs_output() {
		echo $this->render_markup; // WPCS: XSS ok.
	}
	...
	private function get_terms() {
		...
		if ( 'post' == $this->post->post_type ) {
			$taxonomy = 'category';
		} elseif ( 'product' == $this->post->post_type && class_exists( 'WooCommerce' ) && aurora_inside_woocommerce() ) {
		...
		if ( 1 === count( $terms ) ) {
			...
			if ( $term_parent ) {
				$term_tree   = get_parents( $terms[0]->term_id, $taxonomy );
	...
	private function get_parents() {
		...
		$parent_ids = array_reverse( get_parents( $this->post ) );
	...
	private function get_taxonomies() {
		...
		if ( 0 != $term->parent && is_taxonomy_hierarchical( $term->taxonomy ) ) {
			$term_parents = get_parents( $term->term_id, $term->taxonomy );
	...
	private function get_woocommerce_page( $linked = true ) {
		...
		if ( is_object( $post_type_object ) && class_exists( 'WooCommerce' ) && ( aurora_inside_woocommerce() || is_cart() || is_checkout() || is_account_page() ) ) {
	...
	private function get_breadcrumb_trail_markup( $object_type = '' ) {
		...
		switch ( $object_type ) {
			...
			case 'year':
				...
				$title = esc_html__( 'Year: ', 'aurora-wp' ) . $year;
				break;
			case 'month':
				...
				$title = esc_html__( 'Month: ', 'aurora-wp' ) . $wp_locale->get_month( $monthnum );
				break;
			case 'day':
				...
				$title = esc_html__( 'Day: ', 'aurora-wp' ) . $day;
				break;
			case 'author':
				...
				$title = esc_html__( 'Articles Posted by&nbsp;', 'aurora-wp' ) . $user->display_name;
				break;
			case 'search':
